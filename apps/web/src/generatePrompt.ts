...
export const generatePrompt = async ({ prompt, answers = [] }: any, user?: any) => {
  ...
  try {
    ...
    const { data } = await openai.createCompletion({
      ...
      prompt: `
        Return the filters as a vallid JSON object included in the following question. All keys and values must be lowercase. Reply format can be like: 
    ...
    [filteredData, unfilteredData] = await Promise.all([
      filter ? pineconeQuery(embeddings, { filter, topK: 20 }) : { matches: [] },
      pineconeQuery(embeddings, {})
    ]);
    // pineconeData = [...(unfilteredData?.matches || []), ...(filteredData?.matches || [])];
    pineconeData = filteredData?.matches || unfilteredData?.matches || [];

...
const SourcesBasicDocument: React.FC<{
  ...
  const sourceFilterSources: DocumentFilter[] =
    (filters?.datasources?.[source] as StandardDocumentUrlFilters)?.url?.sources ?? [];
      ...
      <Box marginBottom={1} sx={{ display: 'flex', gap: 2, flexDirection: 'column' }}>
        <Autocomplete
          ...
          onChange={(selectedValues) =>
            updateFilter({ datasources: { [source]: { url: { sources: selectedValues } } } })
          }

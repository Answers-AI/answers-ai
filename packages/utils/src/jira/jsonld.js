...
class JiraEndpoint {
  ...
  async fetchData() {
    const response = await fetch(
      `https://lastrev.atlassian.net/rest/api/3/${this.endpoint}`,
      {
...
class JSONLDSchema {
  ...
  getProperties(data) {
    return new Set(
      data.reduce((acc, item) => acc.concat(Object.keys(item)), [])
    );
  }
  ...
  getIds(data, property) {
    return new Set(
      data
        .filter((item) => item[property] && item[property].type)
        .map((item) => item.id)
    );
  }
  ...
  getPropertyValues(data, property) {
    return new Set(
      data.filter((item) => item[property]).map((item) => typeof item[property])
    );
  }
        ...
        if (property === "schema") {
          ...
          ids.forEach((id) => {
            this.schema[endpoint][property][id] = {
              "@id": `jira:${id}`,
              "@type": `xsd:${
                this.data[endpoint].find((item) => item.id === id).schema.type
              }`,
            };
            if (
              this.data[endpoint].find((item) => item.id === id).schema.system
            ) {
              let jiraId = `jira:${id.charAt(0).toUpperCase()}${id.slice(1)}/`;
          ...
          this.schema[endpoint][property]["@type"] =
            propertyValues.size === 1
              ? Array.from(propertyValues)[0]
              : Array.from(propertyValues);
        }
      });
    });
    return {

...
class DetectToc extends ItemTransformer_1.default {
    constructor() {
        super('Detect TOC', 'Detect table of contents.', {
            ...
            producesGlobels: [exports.TOC_GLOBAL.key],
            debug: {
    ...
    transform(context, inputItems) {
        ...
        const pagesToEvaluate = groupingUtils_1.groupByPage(inputItems.filter((item) => item.page <= maxPageToEvaluate));
        ...
        const tocItemUuids = new Set(functional_1.flatten(functional_1.flatten(rawTocEntries.map((e) => e.entryLines))).map((item) => item.uuid));
        const tocHeadline = findTocHeadline(context.fontMap, mostUsedHeight, tocArea, itemsInTocArea, tocItemUuids);
        const isDefined = (items) => {
            return !!items;
        };
        ...
        return {
            items: inputItems
                ...
                if (itemType) {
                    return items_1.itemWithType(item, itemType);
                }
                return item;
            }),
            messages: [
            ...
            globals: [exports.TOC_GLOBAL.value(new TOC_1.default(tocHeadline, tocArea.pages, headlineTypes))],
        };
    }
}
...
function findTocArea(pagesToEvaluate, pageCount, maxPageToBeLinkedTo) {
    ...
    pagesToEvaluate.forEach((pageItems) => {
        const itemsGroupedByLine = groupingUtils_1.groupByLine(pageItems);
    ...
    if (!numberFunctions_1.numbersAreConsecutive(pages)) {
        return undefined;
    }
...
function findEndingNumber(lineItems) {
    const text = lineItems
        ...
        .trim();
    return stringFunctions_1.extractEndingNumber(text);
}
function selectRawTocEntries(tocArea, itemsInTocArea) {
    ...
    const itemsInTocAreaByLine = groupingUtils_1.groupByLine(itemsInTocArea);
    ...
    const maxLinesBetweenLinesWithNumbers = Math.max(...itemsInTocAreaByLine
        ...
        .filter(groupingUtils_1.onlyUniques));
    const linesWithNumbersByPage = functional_1.groupBy(tocArea.linesWithNumbers, (line) => line.page);
...
function findTocHeadline(fontMap, mostUsedHeight, tocArea, itemsInTocArea, tocItemUuids) {
    ...
    const itemsGroupedByLine = groupingUtils_1.groupByLine(firstPageNonTocItems).filter((lineItems) => hasHeadlineSymptoms(fontMap, mostUsedHeight, lineItems));
...
function findTocEntryHeadlineLevels(rawTocEntries) {
    ...
    const height = (entry) => Math.round(items_1.getHeight(entry.entryLines[0][0]));
    ...
    if (allHeights.length > 3) {
        return rawTocEntries.map(() => ItemType_1.default.H2);
    }
    return rawTocEntries.map((entry) => {
        const index = allHeights.indexOf(height(entry));
        return ItemType_1.default.header(index + 2);
    });
}
/**
 * @param fontMap
...
function fineMatchingHeadlineCanditate(tocEntryText, pageItems, fontMap, mostUsedHeight) {
    const itemsByLine = groupingUtils_1.groupByLine(pageItems);
    ...
    for (let lineIdx = 0; lineIdx < itemsByLine.length; lineIdx++) {
        ...
        if (lineInLink && headlineSymptoms) {
            ...
            const newScore = string_similarity_1.compareTwoStrings(newText, tocEntryText);
...
function hasHeadlineSymptoms(fontMap, mostUsedHeight, lineItems) {
    return (items_1.getHeight(lineItems[0]) >= mostUsedHeight + config.minHeadlineDistance ||
        FontType_1.default.declaredFontTypes(items_1.getFontName(fontMap, lineItems[0])).includes(FontType_1.default.BOLD));
}
function normalizeHeadlineChars(lines) {
    const text = functional_1.flatten(lines)
        .map((item) => items_1.getText(item))
        .join('');
    return stringFunctions_1.filterOut(text, [

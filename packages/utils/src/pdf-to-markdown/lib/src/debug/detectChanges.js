...
function detectChanges(tracker, inputItems, outputItems) {
    const oututItemsByPage = groupingUtils_1.groupByPage(outputItems).reduce((map, pageItems) => {
        map.set(pageItems[0].page, pageItems);
        return map;
    }, new Map());
    const mergedItems = [];
    groupingUtils_1.groupByPage(inputItems).forEach((inputPageItems) => {
...
function detectPageChanges(tracker, inputItems, outputItems) {
    ...
    for (let inputIdx = 0; inputIdx < inputItems.length; inputIdx++) {
        ...
        if (positionInOutput < 0) {
            ...
            if ((typesInInput || typesInOutput) && !functional_1.arraysEqual(typesInInput, typesInOutput)) {

...
class SlackMessage {
  ...
  async getInfo() {
    ...
    if (!message) {
      throw new Error(
        `Message not found in channel ${this.channel}: ${this.ts}`
      );
    }
  ...
  async getTidiedInfo() {
    ...
    const tidiedInfo = {
      ...
      dateEdited: messageInfo.edited
        ? SlackObject.convertTimestampToDate(messageInfo.edited.ts)
        : null,
  ...
  async getReplies() {
    ...
    const replies = result.messages.map(
      (m) => new SlackMessage(this.slackApiClient, this.channel, m, this)
    );
